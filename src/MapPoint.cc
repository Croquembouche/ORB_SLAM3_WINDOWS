...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    // Compute distances between them (MSVC-safe: no VLAs)
    ...
    for (size_t i = 0; i < N; ++i) {
        for (size_t j = i + 1; j < N; ++j) {
            const int d = ORBmatcher::DescriptorDistance(vDescriptors[i], vDescriptors[j]);
            Distances[i][j] = d;
            Distances[j][i] = d;
        }
    }
    ...
    size_t BestIdx = 0;
    for (size_t i = 0; i < N; ++i) {
        std::vector<int> vDists = Distances[i];          // copy row i
        std::sort(vDists.begin(), vDists.end());
        const int median = vDists[(N - 1) / 2];          // same as original 0.5*(N-1)
